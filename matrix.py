...
class Matrix(object):
    def __init__(self, rows = 0, cols = 0, copy = None, fractional = False):
        if copy is not None:
        ...
        for m in range(len(copy) if copy else rows):
            self._m.append(Vector(fractional = fractional))
            for n in range(len(copy[m]) if copy else cols):
                if fractional:
                    self._m[m].append(F(copy[m][n] if copy else F(0)))
                else:
    ...
    def simplify(self):
        ...
        while i < len(B):
            ...
            if f < 0:
                i += 1
                continue
            B[i] = self._row_op_scalar_mult(B, i, 1 / B[i][f])
            i += 1
        return B
    ...
    def _gauss_jordan_elimination(self, reduced = True):
        ...
        while i < len(B):
            ...
            while j < len(B):
                ...
                B[j] = self._row_op_factor_add(B, i, j, s * B[j][f] / B[i][f])
